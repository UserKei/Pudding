...
@onready var q_charge_label: Label = %QChargeLabel
...
func _ready() -> void:
	...
	GameState.q_charges_changed.connect(_on_q_charges_changed)
	...
	_on_q_charges_changed(GameState.q_charges)
...
func _on_q_charges_changed(count: int) -> void:
	q_charge_label.text = "Q %d" % count
